...
for i=1:3
    ...
    elseif i==2
        ...
        osimModel=Model('arm_post_transfer_29_length3_edited.osim')
    end
...
%------------------------BRACHIORADIALIS:--------------------------
        ...
        % TOTAL MOMENT
        total_moment=[total_moment;-1*(ecu_m+edci_m+edcl_m+edcm_m+edcr_m+edm_m+eip_m+epl_m+br_moment)];
        ...
        % To find wrist active range of motion:
    ...
    for e=linspace(rMinE,rMaxE,100)
        ...
        ECRB_ma=ECRB.computeMomentArm(state, flexion)*-1; % Wrist flexion moment arm
        ECRB.computeActuation(state); % Force
        ...
        br_moment_el=[br_moment_el;br_moment_a,br_moment_p];
    end
...
ylabel('Force (N)')
legend('Moment Arm 1','Moment Arm 2','Moment Arm 3')
...
legend('Moment Arm 1','Moment Arm 2','Moment Arm 3','ECRB unparalysed')
...
legend('Moment Arm 1','Moment Arm 2','Moment Arm 3','Pre-transfer')
...
figure('name','active passive moment')
tiledlayout('flow')
